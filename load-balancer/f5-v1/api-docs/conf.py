...
extlinks = {
    # TODO: EG - replace some of these with valid endpoints for F5.
...
rst_epilog = """
.. |apiservice| replace:: Dedicated F5 Load Balancer API
...
htmlhelp_basename = 'BPIF5LoadBalancerApiDeveloperDoc'
...
latex_documents = [
    ('index', 'RackspaceManaagedLoadBalancerApiDeveloperDoc.tex',
     'BPI F5 Load Balancer API Developer Guide', 'Rackspace', 'manual')
]
...
man_pages = [
    ('index', 'BPIF5LoadBalancerApiDeveloperDoc',
     'BPI - F5 Load Balancer API Developer Guide', ['Rackspace'], 1)
]
...
texinfo_documents = [
    ('index', 'RackspaceBigDataDeveloperDoc',
     'BPI F5 Load Balancer API Developer Guide', 'Rackspace',
     'BPILoadBalancerAPIDeveloperDoc',
     'Learn about using the REST API for the BPI - F5 Load Balancer service',
     'Miscellaneous'),
]
